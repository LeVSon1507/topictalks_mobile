...
const fetchTopic = async (
   ...
   try {
      const response = await axiosJWT.get(
         `${TOPIC_TALKS_DOMAIN}/topic-${isChildren ? 'children' : 'parent'}/all${
...
export const useGetAllTopicParents = (
   account: IUser,
   setAccount: (account: IUser | null) => void
) => {
...
export const fetchTopicChildren = async (topicParentId: number, axiosJWT, account: IUser) => {
   try {
      const response = await axiosJWT.get(
         `${TOPIC_TALKS_DOMAIN}/topic-children/topic-parent=${topicParentId}`,
         {
            headers: {
...
export const useGetAllTopicChildren = (
   account: IUser,
   setAccount: (account: IUser | null) => void,
   id: number
) => {
...
const fetchAllPostByIsApproved = async (
   ...
   try {
      const response = await axiosJWT.get(
         `${TOPIC_TALKS_DOMAIN}/post/all-posts/is-approved=false`,
         {
            headers: {
               Authorization: `Bearer ${bearerToken}`,
            },
...
export const useGetAllPostsByIsApproved = (
   ...
   return useQuery('post-by-is-approved', () =>
      fetchAllPostByIsApproved(account, setAccount, isApproved)
   );
};
// ================================= Delete post ==============================
...
export const removeLike = async (postId: number, account: IUser) => {
   ...
   const response = await fetch(
      `${TOPIC_TALKS_DOMAIN}/like/remove/uid=${account.id}&&pid=${postId}`,
      {
         method: 'DELETE',
         headers: {
            'Content-Type': 'application/json',
